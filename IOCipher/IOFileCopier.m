...
@interface IOFileCopier () <NSStreamDelegate>

@property (nonatomic, strong) NSNumber *bytesWritten;

@end

@implementation IOFileCopier
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode
{
    switch (eventCode) {
        case NSStreamEventHasBytesAvailable:
        {
            ...
            NSInteger len = 0;
            len = [(NSInputStream *)stream read:buf maxLength:bufferLength];
            if(len) {
...
{
    return [[[self class] allocWithZone:zone] initWithFilePath:self.filePath toEncryptedPath:self.encryptedFilePath ioCipher:self.ioCipher completionQueue:self.completionQueue completion:self.completion];
}
